...
void RSA_Keys::primalityTestAndGeneration(Bignum& result, Bignum& resultPhi, bool longer) {
	...
	while (true) {
		if (BN_num_bits(result.get()) != RSA_MODULUS_BITS / 4 + (longer ? 1 : 0))
			throw std::out_of_range("Prime is not a " + std::to_string(RSA_MODULUS_BITS / 4 + (longer ? 1 : 0)) + "-bit number.");
...
const Bignum RSA_Keys::generatePublicModulus(const Bignum& p, const Bignum& q) {
	...
	std::cerr << BN_num_bits(p.get()) << "\n"
	          << BN_num_bits(q.get())
	          << "\n"
	          << BN_num_bits(n.get()) << std::endl;

	if (BN_num_bits(n.get()) != RSA_MODULUS_BITS / 2 + (isClient ? 0 : 1))
		throw std::out_of_range("Modulus is not a " + std::to_string(RSA_MODULUS_BITS / 2 + (isClient ? 0 : 1)) + "-bit number.");
